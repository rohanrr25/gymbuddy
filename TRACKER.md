...
| 14 | UI/UX polish pass | ✅ `491ed13`. Every screen reviewed against the web-interface guidelines. Most already passed — the standing per-feature review had done its job — so the pass was small: **reduced motion** (unsupported, and this week added animation), **Home's week strip** (a rolling 7 days beside a Monday–Sunday count), a **duplicated "Appearance" label** in Settings, and a straight apostrophe. The per-feature review stays a standing rule. |
...
| 2026-09-22 | **Home's week strip is the week the streak counts** | The strip was a rolling seven days while "1/3 this week" was Monday–Sunday, so three filled squares could sit under the number 1. `currentWeek()` replaces `recentWeek()`: same Monday–Sunday week as `streakWeeks` and `workoutsThisWeek`, with today ringed (matching the calendar) and days you haven't reached yet dimmed rather than drawn as missed. A check asserts the strip's filled count equals `workoutsThisWeek`, so the two can't drift apart again. |
| 2026-09-22 | **Reduced motion is handled once, globally** | Asking a phone for less motion now removes every animation: the PR card's entrance, the loading skeleton's pulse, button transitions. One `@media (prefers-reduced-motion: reduce)` block in `globals.css` rather than a `motion-reduce:` variant per component — the blanket version covers animations written later, which is the failure mode that matters. Accessibility basics are never the thing to simplify away. |
...
| 2026-09-22 | **The logger's list clears when you complete, and only then** | User: "have it clean up after you press complete workout and have everything done today in the current workout stay." The list is everything logged **today since your last completion** — one rule, one thing that clears it. This replaced a first attempt that scoped by the day a set was tagged with, which was wrong in the case that matters: an exercise you pick **off-plan** mid-session is tagged differently, so it vanished from the list the moment you logged it. The completion time comes from the latest **workout** row, not the plan's `lastCompletion`, because the plan only knows routine days and a freestyle session would never clear the list. The tap clears it straight away; **Undo brings the workout and its sets back**. What you already finished stays in its completion summary. |
