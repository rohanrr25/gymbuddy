...
| 13 | Home-screen install | ✅ Built: `app/manifest.ts`, plus `app/icon.tsx` and `app/apple-icon.tsx` generated at build time (no binary assets to keep in sync with the palette), and `appleWebApp` metadata with `viewport-fit: cover`. **Deliberately no web push yet** — that plumbing is what a native app would replace (Direction). |
