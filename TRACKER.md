...
| 5 | Gym flow | Home shows the active routine's **next day by rotation** (the day after the last one trained; overridable) with its exercises ready to log in the weight × reps logger. Free logging of any exercise stays available. | 🟡 Built: migration `003_sets_routine_day.sql`, `getActivePlan()` in `lib/routines.ts`, day tagging and an ownership check in `lib/sets.ts`, `lib/rotation.ts` (+ `npm run check`, 6/6), and the plan section in `app/logger.tsx`. SQL tested with throwaway users (5/5, cleaned up). **Not yet verified:** on the user's phone at the gym. |
